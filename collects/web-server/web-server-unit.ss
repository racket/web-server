                  ...
                  (let-values ([(ip op) (listener)])
                    (let* ([shutdown
                      ...
                      (if ses
                          (thread (lambda ()
                                    (with-handlers (
                                                    ;; GregP I don't know what the new exception should be
                                                    ;; and this code is insufficiently documented and  has no
                                                    ;; tests for this case either. :-(
;                                                    [exn:i/o:port:closed?
;                                                     (lambda (exn)
;                                                       (connection-lost)
;                                                       (kill-this-session))]
      ...
      (define (dispatch top-custodian method host-info uri headers ses)
        (let ([path (url-path->string (url-path uri))]
          ...
          (cond
            ...
            [else
             (report-error conn method ((responders-file-not-found (host-responders host-info)) uri))])))
